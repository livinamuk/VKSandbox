...
SkinnedModelData AssimpImporter::ImportSkinnedFbx(const std::string filepath) {
    ...
    if (!scene) {
        std::cout << "AssimpImporter::ImportSkinnedFbx() failed to loaded model " << filepath << "\n";
        std::cerr << "Assimp Error: " << importer.GetErrorString() << "\n";
        return modelData;
    }
    modelData.name = Util::GetFileName(filepath);
    ...
    for (int i = 0; i < scene->mNumMeshes; i++) {
        ...
        for (unsigned int j = 0; j < assimpMesh->mNumBones; j++) {
            ...
            if (boneMapping.find(boneName) == boneMapping.end()) {
                ...
                boneIndex = currentBoneCount;
                currentBoneCount++;
                // you do want this eventually BoneInfo bi;
                // you do want this eventually skinnedModel->m_BoneInfo.push_back(bi);
                // you do want this eventually skinnedModel->m_BoneInfo[boneIndex].BoneOffset = Util::aiMatrix4x4ToGlm(assimpMesh->mBones[j]->mOffsetMatrix);
                // you do want this eventually skinnedModel->m_BoneInfo[boneIndex].BoneName = boneName;
    ...
    for (int i = 0; i < modelData.meshes.size(); i++) {
        ...
        // Vertices
        for (unsigned int j = 0; j < meshData.vertexCount; j++) {
            meshData.vertices[j] = (WeightedVertex{
                // Pos
                glm::vec3(assimpMesh->mVertices[j].x, assimpMesh->mVertices[j].y, assimpMesh->mVertices[j].z),
                // Normal
                glm::vec3(assimpMesh->mNormals[j].x, assimpMesh->mNormals[j].y, assimpMesh->mNormals[j].z),
        ...
        for (unsigned int i = 0; i < assimpMesh->mNumBones; i++) {
            for (unsigned int j = 0; j < assimpMesh->mBones[i]->mNumWeights; j++) {
                ...
                unsigned int boneIndex = boneMapping[boneName];
        ...
        // Generate Tangents
        for (int i = 0; i < meshData.indices.size(); i += 3) {
            WeightedVertex* vert0 = &meshData.vertices[meshData.indices[i]];
            WeightedVertex* vert1 = &meshData.vertices[meshData.indices[i + 1]];
            WeightedVertex* vert2 = &meshData.vertices[meshData.indices[i + 2]];
            glm::vec3 deltaPos1 = vert1->position - vert0->position;
            glm::vec3 deltaPos2 = vert2->position - vert0->position;
            glm::vec2 deltaUV1 = vert1->uv - vert0->uv;
            glm::vec2 deltaUV2 = vert2->uv - vert0->uv;
