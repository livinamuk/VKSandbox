...
void main() {
   ...
   // noiseValue = noiseValue * noiseValue;
    ...
    grassColor1 = vec3(0.18, 0.38, 0.14);  // Deep natural green
    grassColor2 = vec3(0.12, 0.25, 0.10);  // Darker, earthier green
   

    grassColor1 = vec3(0.18, 0.38, 0.14);  // Deep natural green
   grassColor2 = vec3(0.45, 0.40, 0.12);  // More yellowish, dry grass

   grassColor1 = vec3(0.22, 0.33, 0.18);  // Slightly desaturated green
   ...
   // BaseColorOut.rgb = mix(BaseColorOut.rgb, color, 0.25);
     BaseColorOut.rgb = color;
    BaseColorOut.rgb = mix(BaseColorOut.rgb, vec3(noiseValue * noiseValue), 0.35);
   ...
   // EmissiveOut = vec4(BaseColorOut.rgb, 0);
}
